                               ...
                               std::map<Parameter, std::set<Entity>>& pArgumentsToFilter) const
{
  ...
  /*
  for (const auto& currParam : parametersAlreadyInSetOfFacts)
  {
    auto it = pArgumentsToFilter.find(currParam.first);
    if (it != pArgumentsToFilter.end())
    {
      if (it->second.empty())
        continue;
      for (auto& currEntityToRemove : currParam.second)
        it->second.erase(currEntityToRemove);
      if (it->second.empty())
        return false;
    }
  }
*/
  ...
  if (parametersAlreadyInSetOfFacts != pArgumentsToFilter)
  {
  //  if (!parametersAlreadyInSetOfFacts.empty())
  //      pArgumentsToFilter = std::move(parametersAlreadyInSetOfFacts);
    return true;
  }
